Shader "Custom/s_NoiseWMask"
{
    ...
    Subshader
    {
        ...
        Pass
        {
            ...
            MeshData Interpolators (float4 vertex:POSITION, float2 uv:TEXCOORD0, float2 mask:TEXCOORD1)
            {
                MeshData vs;
            ...
            {  
                ...
                if (m.x < 0.05)
                discard;
                float4 noise = float4(min(c, m.x), min(c, m.y), min(c, m.z), c) * _emission * _emissionColor;
                return noise * m;
            }
            ENDCG
        }
